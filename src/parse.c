...
static Type *declspec(Token **rest, Token *tok, VarAttr *attr) {
  ...
  while (is_typename(tok)) {
    ...
    if (consume(&tok, tok, "const") || consume(&tok, tok, "volatile") ||
        consume(&tok, tok, "var") || consume(&tok, tok, "register") ||
...
static Type *func_params(Token **rest, Token *tok, Type *ty) {
  if (equal(tok, "func") && equal(tok->next, ")")) {
    *rest = tok->next->next;
    return func_type(ty);
  }
...
static bool is_typename(Token *tok) {
  ...
  if (map.capacity == 0) {
    static char *kw[] = {
      "func", "bool", "char", "short", "int", "long", "struct", "union",
      "enum", "static", "extern", "signed", "unsigned",
      "const", "var", "float", "double", "say", "read"
    };
...
static Token *function(Token *tok, Type *basety, VarAttr *attr) {
  ...
  /*if (fn) {
    ...
    fn->is_function = true;
    fn->is_definition = true; //equal(tok, "{");
